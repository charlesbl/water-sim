...
struct PressureGeometry {
    // Bit 0: air cell. Bits 1..6: +x, -x, +y, -y, +z, -z open faces.
    mask: u32,
    diagonal: f32,
};
@group(0) @binding(23) var<storage, read_write> pressureGeometry: array<PressureGeometry>;
...
// Rebuild from the same source volume and columns used by this tick's advection.
// A separate dispatch makes every donor's value available before any flux reads.
@compute @workgroup_size(4, 4, 4)
fn prepareCourant(@builtin(global_invocation_id) id: vec3<u32>) {
    if (!inside(id)) { return; }
    let p = vec3<i32>(id);
    outgoingCourants[index(p)] = outgoingCourant(p);
}
...
struct TransportedScalars {
    water: vec4<f32>,
    temperature: f32,
};

// Both scalar transports use the same face geometry, donor and outgoing CFL.
// Keep their independent reconstructions and the original arithmetic order.
fn scalarFlux(p: vec3<i32>, axis: u32) -> ScalarFlux {
    var offset = vec3<i32>(0);
    offset[axis] = 1;
    if (!air(p) || !air(p + offset)) { return ScalarFlux(vec4<f32>(0.0), 0.0); }
    let speed = volumeIn[index(p)].velocityTemperature[axis];
    let donor = select(p + offset, p, speed >= 0.0);
    let courant = outgoingCourants[index(donor)];
    var water = volumeIn[index(donor)].moisture;
    var potential = potentialTemperature(donor);
    if (courant <= 0.5 && air(donor - offset) && air(donor + offset)) {
        // MUSCL minmod bounds each face by 1.5 times its donor. At combined
        // CFL <= 0.5, total outgoing water stays below 0.75 of the inventory.
        let slope = limitedSlope(volumeIn[index(donor - offset)].moisture,
            water, volumeIn[index(donor + offset)].moisture);
        water += 0.5 * sign(speed) * (1.0 - courant) * slope;
        let before = potential - potentialTemperature(donor - offset);
        let after = potentialTemperature(donor + offset) - potential;
        if (before * after > 0.0) {
            potential += 0.5 * sign(speed) * (1.0 - courant) * sign(before) * min(abs(before), abs(after));
        }
    }
    // High-CFL donor fallback and division are unchanged for both quantities.
    let faceCourant = speed * u.spacingTime.w / u.spacingTime[axis];
    return ScalarFlux(water / max(1.0, courant) * faceCourant,
        potential / max(1.0, courant) * faceCourant);
}

fn transportScalars(p: vec3<i32>) -> TransportedScalars {
    var water = volumeIn[index(p)].moisture;
    var potential = potentialTemperature(p);
    for (var axis = 0u; axis < 3u; axis++) {
        var offset = vec3<i32>(0);
        offset[axis] = 1;
        let incoming = scalarFlux(p - offset, axis);
        let outgoing = scalarFlux(p, axis);
        water += incoming.water - outgoing.water;
        potential += incoming.potential - outgoing.potential;
    }
    // Only roundoff can cross zero; the face donor limiter guarantees positivity.
    return TransportedScalars(max(water, vec4<f32>(0.0)),
        potential - 273.15 - dryLapse * (f32(p.z) + 0.5) * u.spacingTime.z);
}
...
fn advect(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    var temperature = transported.temperature + longwaveHeating[i];
    var water = transported.water;
...
fn divergence(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    pressureOut[i] = 0.0;
    if (!air(p)) {
        pressureGeometry[i] = PressureGeometry(0u, 0.0);
        divergenceField[i] = 0.0;
        cgState[i] = vec4<f32>(0.0);
        return;
    }
    // Columns remain fixed throughout this pressure solve. Rebuild every tick,
    // including when water, snow or ice changes the solid/air boundary.
    let geometry = cellPressureGeometry(p);
    pressureGeometry[i] = geometry;
    let positive = faceVelocity(p);
    ...
    let preconditioned = residual / max(geometry.diagonal, 0.00001);
    cgState[i] = vec4<f32>(residual, preconditioned, preconditioned, 0.0);
}

fn cellPressureGeometry(p: vec3<i32>) -> PressureGeometry {
    var mask = 1u;
    var weights = 0.0;
    for (var axis = 0u; axis < 3u; axis++) {
        var offset = vec3<i32>(0);
        ...
        if (air(p + offset)) { mask |= 1u << (1u + 2u * axis); weights += weight; }
        if (air(p - offset)) { mask |= 1u << (2u + 2u * axis); weights += weight; }
    }
    return PressureGeometry(mask, weights);
}
...
fn cgApply(@builtin(global_invocation_id) id: vec3<u32>,
    ...
    if (i < u32(u.grid.x * u.grid.y * u.grid.z)) {
        ...
        let mask = pressureGeometry[i].mask;
        if ((mask & 1u) != 0u) {
            let state = cgState[i];
            var product = 0.0;
            for (var axis = 0u; axis < 3u; axis++) {
                ...
                offset[axis] = 1;
                let weight = 1.0 / (u.spacingTime[axis] * u.spacingTime[axis]);
                if ((mask & (1u << (1u + 2u * axis))) != 0u) { product += (state.z - cgState[index(p + offset)].z) * weight; }
                if ((mask & (1u << (2u + 2u * axis))) != 0u) { product += (state.z - cgState[index(p - offset)].z) * weight; }
            }
            cgState[i].w = product;
...
fn cgUpdate(@builtin(global_invocation_id) id: vec3<u32>,
    ...
    var product = 0.0;
    if (i < u32(u.grid.x * u.grid.y * u.grid.z)) {
        let geometry = pressureGeometry[i];
        if ((geometry.mask & 1u) != 0u) {
            var state = cgState[i];
            pressureOut[i] += cgCoefficients.x * state.z;
            state.x -= cgCoefficients.x * state.w;
            state.y = state.x / max(geometry.diagonal, 0.00001);
...
fn surfaceExchange(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // Keep wet-snow physics shared with production; this reference isolates
    // only the mapping optimization, not the material model.
