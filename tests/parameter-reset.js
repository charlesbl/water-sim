...
try {
  const doc = new DOMParser().parseFromString(
    await (await fetch(import.meta.env.BASE_URL)).text(),
    'text/html'
  );
  doc.querySelectorAll('script').forEach((s) => s.remove());
  document.body.replaceChildren(
    ...Array.from(doc.body.childNodes, (n) => document.importNode(n, true))
  );
  let terrainResets = 0;
  ...
  setupWeatherControls(() => {
    throw new Error('Individual reset must not reset weather');
  });
  setupCommandUI();
  const reset = (id) => {
  ...
  check(
    'Defaults hide every reset including shortcuts',
    [...document.querySelectorAll('[data-reset-for]')].every(
      (b) => b.style.visibility === 'hidden' && b.disabled
    )
  );
  ...
  check(
    'Reset visibility preserves layout',
  ...
  reset('water-gravity');
  check(
    'Every range, checkbox and select has an individual reset',
    [...document.querySelectorAll('input[type="range"], input[type="checkbox"], select')].every(
      (e) => document.querySelector(`[data-reset-for="${e.id}"]`)
    )
  );
  for (const [id, key, value] of [
    ['water-gravity', 'waterGravity', 20],
    ['cooling-middle-altitude', 'coolingMiddleAltitude', 8],
    ['albedo-strength', 'albedoStrength', 2],
    ['border-mode', 'borderMode', 1],
    ...
    ['weather-enabled', 'weatherEnabled', false],
    ...
    reset(id);
    check(
      `${id} restores the exact default and only that parameter`,
      config[key] === defaultConfig[key] &&
        Object.keys(config).every((k) => k === key || config[k] === before[k])
    );
  }
  check('Terrain default applies regeneration once', terrainResets === 1);
  check(
    'Exact numeric field preserves gravity precision',
    document.getElementById('water-gravity-number').value === '9.81'
  );
  config.weatherView = 2;
  reset('weather-view');
  check(
    'Overlay reset restores the world view',
    config.weatherView === 0 && !config.thermalOverlay
  );
  config.viewOpacity = 0.35;
  reset('view-opacity');
  check(
    'Opacity restores exact default',
    config.viewOpacity === 0.72 && document.getElementById('view-opacity-number').value === '72'
  );
