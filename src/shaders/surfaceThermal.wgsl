...
fn sandCoverage(sand: f32) -> f32 {
    return smoothstep(0.0001, 0.05, max(sand, 0.0));
}

fn materialHeatCapacity(sand: f32, liquid: f32, ice: f32, snow: f32) -> f32 {
    // A shallow dry sand layer reacts faster than exposed rock. Water/ice/snow
    // add their actual reservoir capacities, including buried bottom ice.
    return mix(1.5, 0.6, sandCoverage(sand)) + max(liquid, 0.0) * 8.0
        + max(ice, 0.0) * 5.0 + max(snow, 0.0) * 2.0;
}

fn materialAlbedo(sand: f32, liquid: f32, ice: f32, snow: f32) -> f32 {
    var albedo = mix(0.18, 0.38, sandCoverage(sand));
